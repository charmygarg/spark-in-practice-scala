...
object Ex2TweetMining {
  ...
  def mentionOnTweet(): RDD[String] = {
    ...
    tweets.flatMap(_.text.split(" ")).filter(_.startsWith("@")).filter(!_.endsWith("@"))
  }
  ...
  def countMentions(): RDD[(String, Int)] = {
    ...
    // TODO write code here
    val listOfWords = mentions.map(word => (word, 1))
  ...
  def top10mentions(): Array[(String, Int)] = {
    ...
    // TODO write code here
    val mentionCount = countMentions()
    mentionCount.sortBy(-_._2).take(10)
  }
